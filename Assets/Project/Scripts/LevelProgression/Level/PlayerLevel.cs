...
{
    public class PlayerLevel : MonoBehaviour, IExperienceReceiver
    {
        [Header("Система уровней")]
        public LevelProgressionSO levelProgression;

        [Header("Модификаторы по уровням")]
        public LevelModifierNode[] modifiers; // индекс 0 = уровень 1
        ...
        private void ApplyModifierForLevel(int level)
        {
            ...
            foreach (var mod in modifiers[index])
                mod.ApplyTo(abilities);
        }
        ...
        public void ResetProgress()
        {
            ...
            if (modifiers != null)
            {
                for (int index = modifiers.Length; index >= 0; index--)
                    foreach (var mod in modifiers[index])
                        if (mod != null)
